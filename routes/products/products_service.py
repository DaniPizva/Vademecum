...
def createProduct(data: Dict[str, Any]) -> Tuple[Optional[Product], Any]:
    ...
    with get_db() as db:
        ...
        if generic_id is not None:
            ...
            gen_exist = db.query(Generic)\
                .filter(Generic.id == generic_id)\
                .first()
        ...
        dosage_form = (
            data.get("dosage_form") or ""
        ).strip() or None

        posology = (
            data.get("posology") or ""
        ).strip() or None
        ...
        p = db.query(Product).options(
            ...
            joinedload(Product.family_relation_p)
                .joinedload(Family.description_relation_f)
                .joinedload(
                    Description.therapeutic_group_relation_d
                ),
    ...
    with get_db() as db:
        ...
        if "commercial_name" in data:
            ...
            if not commercial_name:
                ...
                return None, {
                    "commercial_name":
                        "commercial_name cannot be empty"
                }
