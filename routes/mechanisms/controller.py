...
from routes.mechanisms import service
...
def delete(id: int): #especificar que sea como un numero porque todo llega como string 
    result, err = service.toggle_state(id)
    if err:
...
def update(id: int, data_body): #especificar que sea como un numero porque todo llega como string 
    result, err = service.update(id, data_body)
    if err:
